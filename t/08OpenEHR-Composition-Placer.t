...
is($placer1->composition_format, 'STRUCTURED', 
...
ok(my $placer2 = OpenEHR::Composition::Placer->new({
...
eval {
    my $placer3 = OpenEHR::Composition::Placer->new();
};
...
my $path = 'laboratory_result_report/laboratory_test:__TEST__/test_request_details/placer_order_number';

is($flat->{$path . '|issuer'}, $placer1->issuer, "issuer set in FLAT format");
is($flat->{$path . '|type'}, $placer1->type, "type set in FLAT format");
is($flat->{$path . '|assigner'}, $placer1->assigner, "assigner set in FLAT format");
is($flat->{$path}, $placer1->order_number, "order_number set in FLAT format");

ok($placer1->composition_format('STRUCTURED'), "Set composition format to STRUCTURED");
ok(my $struct = $placer1->compose, "Called compose method");

is($struct->{'|assigner'}, $placer1->assigner, "assigner set in STRUCTURED format");
is($struct->{'|issuer'}, $placer1->issuer, "issuer set in STRUCTURED format");
is($struct->{'|type'}, $placer1->type, "type set in STRUCTURED format");
is($struct->{'|id'}, $placer1->order_number, "order_number set in STRUCTURED format");
...
is($raw->{value}->{assigner}, $placer1->assigner, "assigner set in RAW format");
is($raw->{value}->{'@class'}, 'DV_IDENTIFIER', "value class set in RAW format");
is($raw->{'@class'}, 'ELEMENT', "class set in RAW format");
is($raw->{archetype_node_id}, 'at0062', "archetype node id set in RAW format");
is($raw->{name}->{value}, 'Placer order number', "name value set in RAW format");
