...
is($filler1->composition_format, 'STRUCTURED', 
...
ok(my $filler2 = OpenEHR::Composition::Filler->new({
    order_number          => '17V444888',
...
eval {
    my $filler3 = OpenEHR::Composition::Filler->new();
};
...
my $path = 'laboratory_result_report/laboratory_test:__TEST__/test_request_details/filler_order_number';

is($flat->{$path . '|issuer'}, $filler1->issuer, "issuer set in FLAT format");
is($flat->{$path . '|type'}, $filler1->type, "type set in FLAT format");
is($flat->{$path . '|assigner'}, $filler1->assigner, "assigner set in FLAT format");
is($flat->{$path}, $filler1->order_number, "order_number set in FLAT format");

note("Testing STRUCTURED compostion");
ok($filler1->composition_format('STRUCTURED'), "Set composition format to STRUCTURED");
ok(my $struct = $filler1->compose, "Called compose method");

is($struct->{'|assigner'}, $filler1->assigner, "assigner set in STRUCTURED format");
is($struct->{'|issuer'}, $filler1->issuer, "issuer set in STRUCTURED format");
is($struct->{'|type'}, $filler1->type, "type set in STRUCTURED format");
is($struct->{'|id'}, $filler1->order_number, "order_number set in STRUCTURED format");
...
is($raw->{value}->{assigner}, $filler1->assigner, "assigner set in RAW format");
is($raw->{value}->{'@class'}, 'DV_IDENTIFIER', "value class set in RAW format");
is($raw->{'@class'}, 'ELEMENT', "class set in RAW format");
is($raw->{archetype_node_id}, 'at0063', "archetype node id set in RAW format");
is($raw->{name}->{value}, 'Filler order number', "name value set in RAW format");
