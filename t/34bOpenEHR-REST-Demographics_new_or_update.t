...
SKIP: {
    ...
    ok(
        $demog_new->update_or_new( $ehr1->ehr_id ),
        'Using update_or_new for new party info'
    );
    ...
    is(
        $demog_update->party->{firstNames},
        'Tweedle Deedle',
        'Party updated with new value'
    );
    ok(
        $demog_update->update_or_new( $ehr1->ehr_id ),
        'Using update_or_new for existing party info'
    );
    print $demog_update->err_msg if ( $demog_update->err_msg );
    is( $demog_update->action, 'UPDATE',
        'Action is update for existing party info' );
