...
my $labtest = OpenEHR::Composition::LabTest->new(
    ...
    test_status     => 'Final',
    test_status_time => DateTime->new(year => 2017, month => 11, day => 10, hour => 14, minute => 12),
...
note("Begin testing FLAT composition");
{
    ...
    ok($path_report->request_format($labreport->composition_format), 'Set report format for rest client');
...
{
    ok($labreport->composition_format('STRUCTURED'), 'Set STRUCTURED composition format');
    ok(my $struct = $labreport->compose(), 'Request composition');
    my $path_report = OpenEHR::REST::PathologyReport->new();

    ok($path_report->request_format($labreport->composition_format), 'Set report format for rest client');
    is($path_report->request_format, 'STRUCTURED', 'Format set to STRUCTURED');
    ok($path_report->composition(to_json($struct)), 'Add composition to rest client');
#ok($path_report->composer_name('David Ramlakhan'), 'Add composer name to rest client');
    ok($path_report->submit_new($ehrId), 'Submit composition');
    diag($path_report->err_msg) if $path_report->err_msg;
    ok(!$path_report->err_msg, 'No Error Message set');
