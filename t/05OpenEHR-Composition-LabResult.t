...
if ($labtest->magnitude) {
    is($flat->{$path . 'result_value/value2|magnitude'}, 
        $labtest->magnitude, 
        'result_value magnitude Flat format'
    );
...
note("not implemented result_value/_name/_mapping:0/target|code: ",  
...
note("not implemented result_value/_name/_mapping:0|match: ",  
    $flat->{$path . 'result_value/_name/_mapping:0|match'}
);
note("not implemented result_value/value2|magnitude_status: ",  
    $flat->{$path . 'result_value/value2|magnitude_status'}
);
note("not implemented result_value/value2|unit: ",  
    $flat->{$path . 'result_value/value2|unit'}
);
note("not implemented result_value/value: ",  
...
note("not implemented result_value/value2/_normal_range/lower|unit: ",
...
note("not implemented result_value/value2/_normal_range/upper|unit: ",
    $flat->{$path . 'result_value/value2/_normal_range/upper|unit'}
);
note("not implemented result_value/value2|normal_status: ",
...
is($struct->{comment}->[0], $labtest->comment, 
    'comment in structured format'
);
...
is($raw->{name}->{value}, 'Laboratory result', 
...
for my $item (@{$items}) {
    if ($item->{archetype_node_id} eq 'at0001') {
        ...
        is($item->{value}->{value}, $labtest->result_value, 
            'RAW result value'
        );
        ...
        is($item->{name}->{value}, 'Comment', 
            'RAW result comment name'
        );
        is($item->{value}->{value}, $labtest->comment, 
            'RAW result comment value'
        );
    }
    elsif ($item->{archetype_node_id} eq 'at0004') {
        note('Processing result ref range');
        is($item->{name}->{value}, 'Reference range guidance',
            'RAW result ref_range name'
        );
        is($item->{value}->{value}, $labtest->ref_range,
            'RAW result ref_range value'
        );
    }
    elsif ($item->{archetype_node_id} eq 'at0005') {
        note('Processing result status');
        is($item->{name}->{value}, 'Result status',
            'RAW result status name'
        );
        is($item->{value}->{value}, $labtest->status->{value},
            'RAW result status value'
        );
        is($item->{value}->{defining_code}->{code_string},
            $labtest->status->{code}, 'RAW result status code'
        );
    }
}
