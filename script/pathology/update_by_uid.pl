...
sub submit_update {
    ...
    $query->template_id('GEL - Generic Lab Report import.v0');
...
=head2 select_sample_to_report

Returns a ResultSet for the sample to be reported if 
the sample date falls within the start and end dete
period for a Data Request

=cut

sub select_sample_to_report {
...
sub get_sample_id {
    ...
    my $sample_rs = $schema->resultset('PathologySample')->search(
        {
            composition_id => $uid,
        },
        {
    ...
    if ( $sample_rs->count == 1 ) {
        return $sample_rs->first->lab_number;
    }
    else {
        return 0;
    }
